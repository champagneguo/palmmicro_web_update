# encoding: utf-8
"""
银河QMT Socket 下单服务 v3（实盘修复版，参考 arbTest 项目 galaxy.py 实现）

相比 v1/v2 的实盘关键修复：
  1. 【下单代码去后缀】passorder 的 orderType=1101 已指定市场，
     带 .SZ/.SH 后缀在实盘会被拒单，统一去掉后缀再下单。
  2. 【run_time 起始时间】用空串 "" 立即启动 check_orders 定时器；
     之前用 "2020-01-01 09:30:00" 这个具体过去日期，实盘下定时器不触发。
  3. 【回执】下单只回简单 "OK:remark"，不做复杂 JSON。

支持指令：
  PING / QUERY_ACCOUNT / QUERY_POS / QUERY_ORDER / QUERY_DEAL
  BUY,<code>,<volume>,<price>[,<account>]     （code 可带可不带 .SZ/.SH 后缀，服务端统一去后缀）
...
def _strip_market(code: str) -> str:
    """去掉 .SZ/.SH/.BJ 后缀。passorder 1101 已指定市场，带后缀实盘拒单。"""
    return code.strip().split('.')[0].strip()
...
def _process_request(req: QueryRequest, ContextInfo):
    ...
    try:
        if action == 'QUERY_ACCOUNT':
            ...
            code    = _strip_market(parts[1])        # 关键：去 .SZ/.SH 后缀，实盘才不拒单
            ...
            result_str = f"OK:{remark}:{ret}"
            req.complete(result_str)
            log(f"passorder 完成: {action} {code} {volume}@{price} 账号={account} 备注={remark} 订单ID={ret}")
...
def init(ContextInfo):
    ...
    # 关键：起始时间空串 = 立即启动（实盘才能触发 check_orders）
    ContextInfo.run_time("check_orders", "1nSecond", "")
    log("定时器 check_orders 已注册（每秒处理请求队列，立即启动）")
